...
class MongoLib {
  ...
  getTournamentMatchesByMatchDay(collection, matchday){
    const { matchDescription } = matchday;
    return this.connect().then(db =>{
      return db.collection(collection).find({ description : matchDescription }).toArray();
    });
  }
  ...
  updateTableByScore(collection, tournamentID, matchScore) {
